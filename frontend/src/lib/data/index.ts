...
export const retrievePricedProductById = cache(
	async ({
		id,
		regionId,
	}: {
...
export const getProductByHandle = cache(
	async (handle: string): Promise<{ product: PricedProduct }> => {
		const headers = getMedusaHeaders(["products"]);
...
export const getProductsList = cache(
	async ({
		pageParam = 0,
		queryParams,
		countryCode,
...
export const getCollectionsList = cache(
	async (
		offset = 0,
		limit = 100,
...
export const getCollectionByHandle = cache(
	async (handle: string): Promise<ProductCollection> => {
...
export const getCategoriesList = cache(
	async (
		offset = 0,
		limit = 100,
	): Promise<{
...
export const getProductsByCategoryHandle = cache(
	async ({
		pageParam = 0,
		handle,
		countryCode,
