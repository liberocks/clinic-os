...
export class CreateAnamnesis1722075061143 implements MigrationInterface {
	...
	`);
		await queryRunner.query(
			`CREATE INDEX "idx_anamnesis_form_id" ON "anamnesis_form" ("id")`,
		);
	...
	`);
		await queryRunner.query(
			`CREATE INDEX "idx_anamnesis_section_form_id" ON "anamnesis_section" ("form_id")`,
		);
	...
	`);
		await queryRunner.query(
			`CREATE INDEX "idx_anamnesis_question_section_id" ON "anamnesis_question" ("section_id")`,
		);
	...
	`);
		await queryRunner.query(
			`CREATE INDEX "idx_anamnesis_response_form_id" ON "anamnesis_response" ("form_id")`,
		);
		await queryRunner.query(
			`CREATE INDEX "idx_anamnesis_response_customer_id" ON "anamnesis_response" ("customer_id")`,
		);
		await queryRunner.query(
			`CREATE INDEX "idx_anamnesis_response_order_id" ON "anamnesis_response" ("order_id")`,
		);
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE "anamnesis_response" DROP CONSTRAINT "FK_anamnesis_response_order"`,
		);
		await queryRunner.query(
			`ALTER TABLE "anamnesis_response" DROP CONSTRAINT "FK_anamnesis_response_customer"`,
		);
		await queryRunner.query(
			`ALTER TABLE "anamnesis_response" DROP CONSTRAINT "FK_anamnesis_response_form"`,
		);
		await queryRunner.query(
			`ALTER TABLE "anamnesis_question" DROP CONSTRAINT "FK_anamnesis_question_section"`,
		);
		await queryRunner.query(
			`ALTER TABLE "anamnesis_section" DROP CONSTRAINT "FK_anamnesis_section_form"`,
		);
